 ...
 * downloaded. The same point clouds, projected once through the same
 * square-on view onto a 2D canvas — so a phone sees the real forms, not a
...
export function SculptureStatic({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const scale = height / (2 * 5.1 * Math.tan((19 * Math.PI) / 180));
      const camera = 5.1;
      ...
      for (let i = 0; i < COUNT; i += 1) {
        ...
        ctx.beginPath();
        ctx.arc(width / 2 + (x1 * fit + shift) * k * scale, height / 2 - (y2 * fit + lift) * k * scale, (accent ? 1.3 : 0.95) * k, 0, Math.PI * 2);
        ctx.fill();
      }
    };
